...
async def promote(promt):
    ...
    mafiaevent = await edit_or_reply(promt, "banara admin ruk...")
    ...
    try:
        ...
        await mafiaevent.edit("BANA DIYA ADMIN CHAL AB PARTY DE!")
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    mafiaevent = await edit_or_reply(dmod, "RUK DEMOTE KRTA HU BC ...")
    rank = "CUTE"
    ...
    try:
        ...
        await mafiaevent.edit(NO_PERM)
        return
    await mafiaevent.edit("HUI HUI DEMOTED....SED!")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    mafiaevent = await edit_or_reply(bon, "FUCKING THIS FOOL")
    try:
    ...
    if reason:
        ...
        await mafiaevent.edit(f"{str(user.id)} FUCKED!")
    if BOTLOG:
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    mafiaevent = await edit_or_reply(usr, "BHAG MC..")
    try:
...
async def _(event):
    ...
    if c.admin_rights or c.creator:
        ...
        await edit_or_reply(
            event, "I AM NOT ADMIN ....PROMO KRDE VAI"
        )
        await sleep(3)
        try:
            await event.delete()
